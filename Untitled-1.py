...
#-------------------------------------Logic------------------------------------------------#
1) Here i used a star Algorithem to solve the problem to solve the problem
    ...
    for i in range(N):
        for j in range(N):
            ...
            if ele != max1 and ele != max2:
                ...
                tempsum1 = abs(N-1-tp[0])  + abs(N-1-tp[1])
                tempsum2 = abs(N-1-tp[0])  + abs(N-2-tp[1])
...
#--------------------------------A_function function----------------------------------------#
def a_star(count,twm,flag,avoidmove,p1,p2,trm1, trm2,yn):
    count +=1
    # print(count,len(s))
    if twm != trm1 and twm != trm2 :
                ...
                if (move == avoidmove and f == flag) != 1: #avoid going back to previous state
                    ...
                    if child != -1: #checking if move is possible or not
                        if  child not in s:
                            temph = heu2(child)
                            if f ==1:
                                temptuple = (temph,child,f,oppmove(move),nextp(p,move),p2,copy.deepcopy(yn) + 1)
                            else:
                                temptuple = (temph,child,f,oppmove(move),p1, nextp(p,move),copy.deepcopy(yn) + 1)
        ...
        if(q.empty() != 1): #if queue not empty
            o_temptuple = q.get() #pop element
            o_child = temptuple[1]
            o_flag = temptuple[2]
            o_avoidmove = temptuple[3]
            o_p1 = temptuple[4]
            o_p2 = temptuple[5]
            o_yn = temptuple[6]
            return a_star(count,copy.deepcopy(o_child),o_flag,o_avoidmove,copy.deepcopy(o_p1),copy.deepcopy(o_p2),trm1,trm2,o_yn) #call a_star again
    else:
        print("\n\n\n **************TASK IS DONE*************")
        return twm
...
#----------------------------------------------------Calling a_star------------------------------------------------------#
temp_rm = a_star(count,copy.deepcopy(wrong_matrix),initialflag,Avoid_move_initial, copy.deepcopy(initialp1),copy.deepcopy(initialp2), rm1,rm2,copy.deepcopy(initial_yn)) #call a_star
